...
from regulus.perturb.representation import validate_input_matrix
...
class PerturbationDataset(Dataset):
    """Aligned gene/CFO matrices with optional perturbation labels.

    No control mean, baseline subtraction or log-fold transformation occurs
    here. Upstream preprocessing owns those choices.
    """
    ...
    def __init__(
        ...
        *,
        ...
        if require_labels and "perturbation" not in self.adata.obs:
            raise ValueError("training h5ad requires obs['perturbation']")
        if "perturbation" in self.adata.obs and not include_controls:
            labels = self.adata.obs["perturbation"].astype(str)
    ...
    def _gene_row(self, row: int) -> np.ndarray:
        ...
        return validate_input_matrix(aligned, self.representation, name="gene matrix")
    ...
    def __getitem__(self, index: int) -> dict:
        ...
        if self.cfo_matrix is not None:
            cfo = torch.from_numpy(np.asarray(self.cfo_matrix[row], dtype=np.float32))
